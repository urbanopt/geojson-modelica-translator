...
model UndisturbedSoilTemperature "Undisturbed soil temperature"
  ...
  parameter Modelica.Units.SI.Duration cosPha1(displayUnit="d")
    "First phase lag of soil surface temperature, in days";
  parameter Modelica.Units.SI.Duration cosPha2(displayUnit="d")
...
equation
  T = TSurMea - TSurAmp1 * exp(pha1) * cos(2*pi*(time/60/60/24-cosPha1)/Year + pha1)
      - TSurAmp2 * exp(pha2) * cos(4*pi*(time/60/60/24-cosPha2)/Year + pha2);
