...
equation
  ...
  if not use_TSet and not on then
    TSet_internal=293.15;
  end if;
  connect(TSet,TSet_internal);
  if not use_X_wSet and not on then
    X_wSet_internal=0.01;
  end if;
  connect(X_wSet,X_wSet_internal);
  if(use_TSet and on and energyDynamics == Modelica.Fluid.Types.Dynamics.SteadyState) or(use_X_wSet and massDynamics == Modelica.Fluid.Types.Dynamics.SteadyState) then
  ...
  if use_TSet and on and energyDynamics <> Modelica.Fluid.Types.Dynamics.SteadyState then
    der(
      T)=(TSet_internal-T)*k/tau;
  else
    T=TSet_internal;
  end if;
  if use_X_wSet and on and massDynamics <> Modelica.Fluid.Types.Dynamics.SteadyState then
    der(
