...
model BoilerStage "Stage controller for boilers"
  ...
  Modelica.Blocks.Interfaces.BooleanOutput y_On[numBoi]
    "On signal of the boilers" annotation (Placement(transformation(extent={{100,16},
            {120,36}}),     iconTransformation(extent={{100,16},{120,36}})));
  ...
  Modelica.Blocks.Interfaces.RealInput TDisSup(unit="K", displayUnit="degC")
    "Heating water supply temperature( distrcit side)."
    annotation (Placement(
        transformation(extent={{-120,16},{-100,36}}),   iconTransformation(
          extent={{-120,16},{-100,36}})));
  Modelica.Blocks.Interfaces.RealInput TDisRet(unit="K", displayUnit="degC")
    "Heating water return temperature( distrcit side)."
    annotation (Placement(
        transformation(extent={{-120,44},{-100,64}}),  iconTransformation(
          extent={{-120,44},{-100,64}})));
  Modelica.Blocks.Interfaces.RealInput mHeaDis(unit="kg/s")
  ...
  Modelica.Blocks.Interfaces.RealOutput QLoa(unit="W")
    "Total heating loads"
    annotation (Placement(transformation(extent={{100,70},{120,90}}),
        iconTransformation(extent={{100,70},{120,90}})));

equation
  connect(off.outPort[1], offToOne.inPort) annotation (Line(points={{-52,45.5},{
          -52,30}},               color={0,0,0}));
  connect(oneToOff.outPort, off.inPort[1]) annotation (Line(points={{-12,27.5},{
          -12,74},{-52,74},{-52,67}},     color={0,0,0}));
  connect(oneToTwo.outPort, twoOn.inPort[1]) annotation (Line(points={{-52,-55.5},
          {-52,-73}},                                                                         color={0,0,0}));
  connect(twoOn.outPort[1], twoToOne.inPort) annotation (Line(points={{-52,-94.5},
          {-52,-102},{-2,-102},{-2,-58}}, color={0,0,0}));
  connect(twoToOne.outPort, oneOn.inPort[2]) annotation (Line(points={{-2,-52.5},
          {-2,6},{-51.5,6},{-51.5,-3}},    color={0,0,0}));
  connect(offToOne.outPort, oneOn.inPort[1]) annotation (Line(points={{-52,24.5},
          {-52,-3},{-52.5,-3}},          color={0,0,0}));
  connect(oneOn.outPort[2], oneToOff.inPort) annotation (Line(points={{-51.75,-24.5},
          {-51.75,-34},{-12,-34},{-12,22}}, color={0,0,0}));
  connect(oneOn.outPort[1], oneToTwo.inPort) annotation (Line(points={{-52.25,-24.5},
          {-52.25,-32},{-52,-32},{-52,-50}}, color={0,0,0}));
  connect(comTab.y, y)
    annotation (Line(points={{89,-14},{100,-14},{100,-20},{110,-20}},
                                              color={0,0,127}));
  connect(comTab.u, intToRea.y)
    annotation (Line(points={{66,-14},{58,-14}},
                                             color={0,0,127}));
  connect(addInt.u2, booToInt1.y) annotation (Line(points={{56,-70},{46,-70},{46,
          -84},{40,-84}}, color={255,127,0}));
  connect(oneOn.active, booToInt.u) annotation (Line(points={{-41,-14},{2,-14},{
          2,-44},{16,-44}},
                          color={255,0,255}));
  connect(twoOn.active, booToInt1.u) annotation (Line(points={{-41,-84},{16,-84}}, color={255,0,255}));
  connect(booToInt.y, addInt.u1) annotation (Line(points={{40,-44},{44,-44},{44,
          -58},{56,-58}}, color={255,127,0}));
  connect(addInt.y, intToRea.u) annotation (Line(points={{80,-64},{92,-64},{92,-28},
          {32,-28},{32,-14},{34,-14}},
                                   color={255,127,0}));
  connect(comTab.y, boiOn.u) annotation (Line(points={{89,-14},{90,-14},{90,6},{
          58,6},{58,26},{64,26}},
                                color={0,0,127}));
  connect(boiOn.y, y_On)
    annotation (Line(points={{87,26},{110,26}}, color={255,0,255}));
  connect(dT.y,pro. u1)  annotation (Line(points={{-39,112},{-13,112},{-13,116},
          {-2,116}},
        color={0,0,127}));
  connect(pro.y,cp. u)  annotation (Line(points={{21,110},{38,110}},
        color={0,0,127}));
  connect(TDisSup, dT.u1) annotation (Line(points={{-110,26},{-88,26},{-88,118},
          {-62,118}}, color={0,0,127}));
  connect(TDisRet, dT.u2) annotation (Line(points={{-110,54},{-80,54},{-80,106},
          {-62,106}}, color={0,0,127}));
  connect(mHeaDis, pro.u2) annotation (Line(points={{-110,86},{-70,86},{-70,88},
          {-20,88},{-20,104},{-2,104}}, color={0,0,127}));
  connect(cp.y, QLoa) annotation (Line(points={{61,110},{86,110},{86,80},{110,80}},
        color={0,0,127}));
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,
            -100},{100,140}})),           Icon(coordinateSystem(
