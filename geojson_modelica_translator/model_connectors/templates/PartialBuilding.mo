...
partial model PartialBuilding "Partial class for building model"
  ...
  Modelica.Fluid.Interfaces.FluidPorts_a secHeaSup[nPorts_aHea](
    redeclare each package Medium = Medium,
    each m_flow(min=if allowFlowReversal then -Modelica.Constants.inf else 0),
    each h_outflow(start=Medium.h_default, nominal=Medium.h_default)) if have_heaLoa
    "Source side inlet ports"
      annotation (Placement(transformation(extent={{-310,-8},{-290,72}}),
        iconTransformation(extent={{-310,-220},{-290,-140}})));
  Modelica.Fluid.Interfaces.FluidPorts_b secHeaRet[nPorts_bHea](
    redeclare each package Medium = Medium,
    each m_flow(max=if allowFlowReversal then +Modelica.Constants.inf else 0),
    each h_outflow(start=Medium.h_default, nominal=Medium.h_default)) if have_heaLoa
    "Source side outlet ports"
      annotation (Placement(transformation(extent={{290,-8},{310,72}}),
        iconTransformation(extent={{290,-220},{310,-140}})));         //if have_heaLoa
  Modelica.Fluid.Interfaces.FluidPorts_a secCooSup[nPorts_aCoo](
    redeclare each package Medium = Medium,
    each m_flow(min=if allowFlowReversal then -Modelica.Constants.inf else 0),
    each h_outflow(start=Medium.h_default, nominal=Medium.h_default)) if have_cooLoa
    "Source side inlet ports"
      annotation (Placement(transformation(extent={{-310,-70},{-290,10}}),
        iconTransformation(extent={{-310,-128},{-290,-48}})));
  Modelica.Fluid.Interfaces.FluidPorts_b secCooRet[nPorts_bCoo](
    redeclare each package Medium = Medium,
    each m_flow(max=if allowFlowReversal then +Modelica.Constants.inf else 0),
    each h_outflow(start=Medium.h_default, nominal=Medium.h_default)) if have_cooLoa
    "Source side outlet ports"
      annotation (Placement(transformation(extent={{290,-70},{310,10}}),
        iconTransformation(extent={{290,-130},{310,-50}})));
...
initial equation
  assert(nPorts_aHea == nPorts_bHea,
    "In " + getInstanceName() +
    ": The numbers of secondary heating source side inlet ports (" + String(nPorts_aHea) +
    ") and outlet ports (" + String(nPorts_bHea) + ") must be equal.");
  assert(nPorts_aCoo == nPorts_bCoo,
    "In " + getInstanceName() +
    ": The numbers of secondary cooling source side inlet ports (" + String(nPorts_aCoo) +
    ") and outlet ports (" + String(nPorts_bCoo) + ") must be equal.");
annotation (
...
First implementation.
  ...
  graphics={Rectangle(
        ...
        Rectangle(
          extent={{10,-186},{290,-202}},
          lineColor={0,0,255},
          pattern=LinePattern.None,
          fillColor={0,0,255},
          fillPattern=FillPattern.Solid),
        Rectangle(
          extent={{-290,-204},{-10,-188}},
          lineColor={0,0,255},
          pattern=LinePattern.None,
          fillColor={255,0,0},
          fillPattern=FillPattern.Solid),
        Rectangle(
          extent={{10,-100},{290,-84}},
          lineColor={0,0,255},
          pattern=LinePattern.None,
          fillColor={255,0,0},
          fillPattern=FillPattern.Solid),
