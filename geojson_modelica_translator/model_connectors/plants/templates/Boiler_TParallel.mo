...
model Boiler_TParallel
  "Multiple identical boiler"
    ...
    Modelica.Blocks.Interfaces.RealOutput TBoiLvg[num](unit="K", displayUnit="degC")
    ...
    Buildings.Fluid.Sensors.TemperatureTwoPort senTDisSup(
      ...
      annotation (Placement(transformation(extent={{-10,-10},{10,10}},
          rotation=0,
          origin={76,0})));
    Modelica.Blocks.Interfaces.RealInput THeaWatSet "Heating water set point."
      annotation (Placement(transformation(
          extent={{-10,-10},{10,10}},
          rotation=0,
  ...
  equation
   for i in 1:numBoi loop
    ...
    connect(on, booToRea.u) annotation (Line(points={{-110,50},{-94,50}}, color={255,0,255}));
    connect(boi.port_b, val.port_a) annotation (Line(points={{18,0},{36,0}},color={0,127,255}));
    connect(boi.T, TBoiLvg) annotation (Line(points={{19,8},{20,8},{20,40},{110,40}},
          color={0,0,127}));
    connect(booToRea.y, filter.u) annotation (Line(points={{-71,50},{-60,50},{-60,
            84},{-55.2,84}}, color={0,0,127}));
    connect(booToRea.y, pro.u1) annotation (Line(points={{-71,50},{-60,50},{-60,34},
            {-38,34}}, color={0,0,127}));
    connect(pro.y, boi.y) annotation (Line(points={{-15,28},{-12,28},{-12,8},{-4,8}},
          color={0,0,127}));
    connect(PI_TBoiLvg.y, pro[1].u2) annotation (Line(points={{-51,-30},{-44,-30},
            {-44,22},{-38,22}}, color={0,0,127}));
    connect(PI_TBoiLvg.y, pro[2].u2) annotation (Line(points={{-51,-30},{-44,-30},
            {-44,22},{-38,22}}, color={0,0,127}));
    connect(port_b, senTDisSup.port_b) annotation (Line(points={{100,0},{86,0}}, color={0,127,255}));
    connect(senTDisSup.T, PI_TBoiLvg.u_m) annotation (Line(points={{76,11},{76,18},
            {62,18},{62,-74},{-62,-74},{-62,-42}}, color={0,0,127}));
    connect(THeaWatSet, PI_TBoiLvg.u_s) annotation (Line(points={{-110,-60},{-92,
            -60},{-92,-30},{-74,-30}}, color={0,0,127}));
