...
def flip_matrix(list_of_dictionaries):
    ...
    for i_dict, dictionary in enumerate(list_of_dictionaries):
        for key_lvl0, value_lvl0 in dictionary.items():
            ...
            if not flipped_dictionary.get(key_lvl0):
                flipped_dictionary[key_lvl0] = {}
            ...
            for key_lvl1, value_lvl1 in value_lvl0.items():
                ...
                if not flipped_dictionary[key_lvl0].get(key_lvl1, None):
                    flipped_dictionary[key_lvl0][key_lvl1] = {}
    ...
    for i_gfun, gfunction in enumerate(list_of_gfunctions):
        ...
        for i in range(len(gfunction)):
            gfunction[gfunction.columns[0]].iloc[i] = (
                ...
                * lst_length_of_boreholes[i_gfun] ** 2
            ...
            gfunction[gfunction.columns[1]].iloc[i] = gfunction[gfunction.columns[1]].iloc[i] / (
                lst_number_of_boreholes[i_gfun]
                * 2
                * math.pi
                * lst_length_of_boreholes[i_gfun]
                * sys_params_ghe_parameters["soil"]["conductivity"]
            )

        # add zeros to the first row
        new_row = pd.Series({gfunction.columns[0]: 0, gfunction.columns[1]: 0})
        gfunction = pd.concat([gfunction.iloc[:0], pd.DataFrame([new_row]), gfunction.iloc[0:]]).reset_index(drop=True)  # noqa: PLW2901
    ...
    for ghe_id_instance in dictionary_ghe_ids:
        if Path(sys_params_ghe_parameters["ghe_dir"]).expanduser().is_absolute():
            list_of_gfunctions.append(
                pd.read_csv(
                    Path(sys_params_ghe_parameters["ghe_dir"]) / ghe_id_instance / "Gfunction.csv",
                    header=0,
                    usecols=[0, 2],
                )
            )
            ...
            try:
                list_of_gfunctions.append(
                    pd.read_csv(
                        sys_param_dir / sys_params_ghe_parameters["ghe_dir"] / ghe_id_instance / "Gfunction.csv",
                        header=0,
                        usecols=[0, 2],
                    )
                )
                ...
                raise SystemExit(
                    ...
                    f'\'{sys_params_ghe_parameters["ghe_dir"]}\' must be relative to the dir your sys-param file is in.'
                )
...
def move_bldg_ghe_grp_diagram_extent_or_points(
    ...
    num_bor_fie_in_grp=None,
):
    # "borefield_stub" temporary solution (OpenModelica)
    # component_type ("District System", "Building", "diagram", or "borefield_stub")
    ...
    if adj_ghe_num_grp_for_diagram_only is None:
    ...
    for key, value in dictionary.items():
        for idx, point in enumerate(value[value_key]):
            ...
            if num_bor_fie_in_grp is None:
                point_y = point[1] + (delta_y_coo * adj_ghe_num_grp_for_diagram_only)
            elif num_bor_fie_in_grp == 0:
                point_y = point[1]
            else:
                point_y = point[1] + (delta_y_coo * num_bor_fie_in_grp)
            ...
            if idx == 0:
                ...
                if connection_boolean:
                    if "points_no_change" in dictionary[key]:
                        point_value = (
                            point_value
                            + str(point_x)
                            + ","
        ...
        if component_type == "Building":
            ...
            if num_bor_fie_in_grp is None:
                ...
                self.ghe_groups_by_num[num_of_bldg_ghe_group]["borefields_diagram"][num_bor_fie_in_grp][annotation_key][
                    key
...
def move_bldg_ghe_grp_diagram_origin(
    ...
    # component_type ("District System" or "district_loop")
    # (may also be "Building" or "diagram" but for now they are not used)
...
class District:
    ...
    def to_modelica(self):
        ...
        sys_params_ghe_parameters = self.system_parameters.get_param(
            "$.district_system.fifth_generation.ghe_parameters"
        )
        ...
        if self._scaffold.project_name == "district_multiple_ghe":
            for num_of_bldg_group in range(self.num_of_bldg_groups):
                ...
                for bldg_id_in_grp in self.bldg_groups_by_num[num_of_bldg_group]["list_bldg_ids_in_group"]:
                ...
                for i_idx, i_value in enumerate(
                    flipped_sys_params_buildings["load_model_parameters"]["time_series"]["filepath"]
                ):
                    ...
                    time_series_file_path = (
                        "modelica://"
                        + self._scaffold.project_name
                        + "/Loads/Resources/Data/B"
                        + flipped_sys_params_buildings["geojson_id"][i_idx]
                        + "/"
                        + mos_file_path_split[1]
                    )
                    list_of_time_series_file_paths.append(time_series_file_path)

                self.bldg_groups_by_num[num_of_bldg_group]["list_of_time_series_file_paths"] = (
                    list_of_time_series_file_paths
                )

        if self._scaffold.project_name == "district_multiple_ghe":
            # creates lists of ghe system parameters of interest (e.g., "length_of_boreholes") by group number
            # E.g., if there are two ghes within one group, the list of ghe system parameters will contain two
            # values of "length_of_boreholes"
        ...
        if self._scaffold.project_name == "district_multiple_ghe":
            for num_of_ghe_group in range(self.num_of_ghe_groups):
                ...
                if sys_params_ghe_parameters["design"]["flow_type"] == "system":
                    self.ghe_groups_by_num[num_of_ghe_group]["lst_borehole_design_flow_rate_in_group"] = [
                        sys_params_ghe_parameters["design"]["flow_rate"] / x
                        for x in self.ghe_groups_by_num[num_of_ghe_group]["lst_number_of_boreholes_in_group"]
                    ]
                else:
                    self.ghe_groups_by_num[num_of_ghe_group]["lst_borehole_design_flow_rate_in_group"] = [
                        sys_params_ghe_parameters["design"]["flow_rate"]
                    ] * number_of_borefields_in_grp
        else:
            # number of unique borefields in system_params_ghe.json
            ...
            if sys_params_ghe_parameters["design"]["flow_type"] == "system":
                design_flow_rate = [
                    sys_params_ghe_parameters["design"]["flow_rate"] / x
                    for x in flipped_sys_params["borehole"]["number_of_boreholes"]
                ]
            else:
        ...
        if self._scaffold.project_name == "district_multiple_ghe":
            for num_of_ghe_group in range(self.num_of_ghe_groups):
                list_ghe_ids_in_group = self.ghe_groups_by_num[num_of_ghe_group]["list_ghe_ids_in_group"]
                # returns number of rows in Gfunction.csv and the pd.read_csv of Gfunction.csv
                (
                    self.ghe_groups_by_num[num_of_ghe_group]["ghe_dir_gfunction_file_rows"],
                    self.ghe_groups_by_num[num_of_ghe_group]["list_of_gfunctions"],
                ) = process_gfunction_file(self, list_ghe_ids_in_group, sys_params_ghe_parameters)
        ...
        if self._scaffold.project_name == "district_multiple_ghe":
            ...
            diagram_ghe_connections_dict = {
                "disStoGrp_pumStoGrp": {
                    "points": [[-112, -60], [-112, -80], [-160, -80], [-160, -100], [-150, -100]],
                    "color": ",color={0,127,255}",
                },
            ...
            self.diagram_single_component_dict = {
                "district_loop": {"Placement": {}},
                "diagram": {"Placement": {}},
            }

            for num_of_bldg_group in range(self.num_of_bldg_groups):
                self.bldg_groups_by_num[num_of_bldg_group]["Placement"] = {}
                self.bldg_groups_by_num[num_of_bldg_group]["Line"] = {}
            ...
            for num_of_ghe_group in range(self.num_of_ghe_groups):
                self.ghe_groups_by_num[num_of_ghe_group]["Placement"] = {}
                self.ghe_groups_by_num[num_of_ghe_group]["Line"] = {}
                ...
                for idx_borefield_2 in range(self.ghe_groups_by_num[num_of_ghe_group]["num_ghes_in_group"]):
                    ...
                    move_bldg_ghe_grp_diagram_extent_or_points(
                        ...
                        component_type="District System",
                        adj_ghe_num_grp_for_diagram_only=adj_ghe_num_grp_for_diagram_only,
                        num_bor_fie_in_grp=idx_borefield_2,
                    )
        ...
        if self._scaffold.project_name == "district_multiple_ghe":
            for num_of_ghe_group in range(self.num_of_ghe_groups):
                self.ghe_groups_by_num[num_of_ghe_group]["list_of_gfunction_file_paths"] = process_gfunction(
                    ...
                    b_modelica_path,
                    self._scaffold.project_name,
                    self.ghe_groups_by_num[num_of_ghe_group]["list_ghe_ids_in_group"],
                )
        ...
        for coupling in list_of_updated_couplings:
            ######################
            # !!!!important!!!! the following if statement is MOST PROBABLY WRONG,
            # however, it is used as a temporary solution
            # This is a coupling despite what it says, this is a work around needed to create arrays
            # of time series for buildings
            ######################
            if self._scaffold.project_name == "district_multiple_ghe":
                template_context = {
        ...
        if self._scaffold.project_name == "district_multiple_ghe":
            for model_i in district_template_params["models"]:
                ...
                district_template_params["list_instance_template_files_of_models"].append(
                    instance_template_path_split[1]
                )
