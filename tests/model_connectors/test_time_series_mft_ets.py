...
from geojson_modelica_translator.model_connectors.load_connectors.time_series_mft_ets_coupling import TimeSeriesMFT
from geojson_modelica_translator.model_connectors.networks import NetworkChilledWaterStub, NetworkHeatedWaterStub
...
class TimeSeriesModelConnectorSingleBuildingMFTETSTest(TestCaseBase):
    def setUp(self):
        ...
        filename = Path(self.data_dir) / "time_series_system_params_massflow_ex1.json"
    ...
    def test_mft_time_series_to_modelica_and_run(self):
        ...
        # Check the results
        results_dir = (
            f"{self.district._scaffold.project_path}/time_series_massflow.Districts.DistrictEnergySystem_results"
        )
        ...
        assert ts_hea_load.min() == 0
        assert ts_hea_load.max() == pytest.approx(52, rel=1)
        assert ts_hea_load.mean() == pytest.approx(4, rel=1)

        assert ts_chi_load.min() == 0
        assert ts_chi_load.max() == pytest.approx(67, rel=1)
        assert ts_chi_load.mean() == pytest.approx(4, rel=1)

        assert cool_q_flow.min() == pytest.approx(-131.9, rel=10)
        assert cool_q_flow.max() == pytest.approx(512936, rel=10)
        assert cool_q_flow.mean() == pytest.approx(30210, rel=10)

        assert heat_q_flow.min() == pytest.approx(-342201, rel=10)
        assert heat_q_flow.max() == pytest.approx(62995, rel=10)
        assert heat_q_flow.mean() == pytest.approx(-13905, rel=10)
