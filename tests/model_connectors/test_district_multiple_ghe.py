...
def add_bldg_ghe_group_num(connected_bldgs_and_ghes_by_order_in_loop, feature_type, group_key):
    ...
    # add group_key to connected_fea_typ_by_order_in_loop dictionary (building groups are separated by ghe, and ghe groups are separated by buildings)
    ...
    for idx_connected_fea_typ_1, key_connected_fea_typ_1 in enumerate(list_of_sorted_keys_connected_fea_typ):
        ...
        # this will not give them the same group number, instead the group number will keep counting up for each group after the first group
        # (i.e., group number for the last group will be higher than for the first group)
    ...
    for idx_connected_fea_typ_2, key_connected_fea_typ_2 in enumerate(list_of_sorted_keys_connected_fea_typ):
        # this is true when first feature in the loop is of the feature_type (e.g., "District System")
        if key_connected_fea_typ_2 == 0:
            # this is true when last feature in the loop is of the feature_type (e.g., "District System")
            if connected_bldgs_and_ghes_by_order_in_loop[list_of_sorted_keys_connected_bldgs_ghes[-1]]["type"] == feature_type:
                group_key_to_reset = connected_bldgs_and_ghes_by_order_in_loop[list_of_sorted_keys_connected_bldgs_ghes[-1]][group_key]
                fea_typ_group_counter -= 1
                boolean_group_is_wrapping = True

        if boolean_group_is_wrapping:
            if connected_bldgs_and_ghes_by_order_in_loop[key_connected_fea_typ_2][group_key] == group_key_to_reset:
    ...
    for idx_connected_bldg_ghe, key_connected_bldg_ghe in enumerate(list_of_sorted_keys_connected_bldgs_ghes):
        if connected_bldgs_and_ghes_by_order_in_loop[key_connected_bldg_ghe]["type"] == feature_type:
            ...
            if connected_bldgs_and_ghes_by_order_in_loop[previous_key_connected_bldg_ghe]["type"] == feature_type:
                ...
                connected_bldgs_and_ghes_by_order_in_loop[key_connected_bldg_ghe][group_in_key] = connected_bldgs_and_ghes_by_order_in_loop[previous_key_connected_bldg_ghe][key_to_return_group_num]
...
def dict_bldg_ghe_group_by_num(connected_bldgs_and_ghes_by_order_in_loop, num_of_fea_typ_groups, num_fea_typ_in_group_key, feature_type, group_key, lst_fea_typ_ids_in_group_key, boolean_group_is_wrapping, group_in_key):
    ...
    for num_of_fea_typ_group in range(num_of_fea_typ_groups):
                ...
                if connected_bldgs_and_ghes_by_order_in_loop[key_connected_bldg_ghe][group_key] == num_of_fea_typ_group:
                    if boolean_group_is_wrapping:
                        if boolean_separate_first_fea_typ_ids_wrapping:
                            lst_first_in_loop_fea_typ_ids_wrapping.append(connected_bldgs_and_ghes_by_order_in_loop[key_connected_bldg_ghe]["id"])
                        else:
                            lst_fea_typ_ids_in_group.append(connected_bldgs_and_ghes_by_order_in_loop[key_connected_bldg_ghe]["id"])
                    else:
                        lst_fea_typ_ids_in_group.append(connected_bldgs_and_ghes_by_order_in_loop[key_connected_bldg_ghe]["id"])
            else:
                # this happens the first time there is a switch in feature type when a group is wrapping
                if boolean_group_is_wrapping:
        ...
        if num_of_fea_typ_group == 0 and boolean_group_is_wrapping:
            fea_typ_groups_by_num[num_of_fea_typ_group][lst_fea_typ_ids_in_group_key] = lst_fea_typ_ids_in_group + lst_first_in_loop_fea_typ_ids_wrapping
        else:
            fea_typ_groups_by_num[num_of_fea_typ_group][lst_fea_typ_ids_in_group_key] = lst_fea_typ_ids_in_group

        fea_typ_groups_by_num[num_of_fea_typ_group][num_fea_typ_in_group_key] = len(fea_typ_groups_by_num[num_of_fea_typ_group][lst_fea_typ_ids_in_group_key])
        ...
        for key_connected_bldg_ghe in list_of_sorted_keys_connected_bldgs_ghes:
            ...
            if connected_bldgs_and_ghes_by_order_in_loop[key_connected_bldg_ghe]["id"] == fea_typ_groups_by_num[num_of_fea_typ_group][lst_fea_typ_ids_in_group_key][0]:
                fea_typ_groups_by_num[num_of_fea_typ_group][group_in_key] = connected_bldgs_and_ghes_by_order_in_loop[key_connected_bldg_ghe][group_in_key]
...
class DistrictSystemTest(TestCaseBase):
    def setUp(self):
        ...
        # start code block 1
        # a function that manipulates the geojson dictionary might simplify this code (e.g., features by id, "ThermalConnector" by startFeatureId, etc)
        # this needs 1) validation like buildings validation within urbanopt_geojson.py, and 2) checking if code belongs here or in another python module.
        ...
        for thermal_junction_1 in self.gj.thermal_junctions:
            ...
            if "is_ghe_start_loop" in thermal_junction_1.feature["properties"] and thermal_junction_1.feature["properties"]["is_ghe_start_loop"]:
        ...
        # returns "Building" and "District System" data using order of feature in the loop (highest-level key of dictionary is order in loop)
        ...
        # return first thermal connector id (continues filling the data for the first "Building" and "District System" in the loop)
        for thermal_connector_1 in self.gj.thermal_connectors:
            if thermal_connector_1.feature["properties"]["startFeatureId"] == start_bldg_or_ghe_id:
                connected_bldgs_and_ghes_by_order_in_loop[0]["id_ThermalConnector_out"] = thermal_connector_1.feature["properties"]["id"]

        # returns "Building" and "District System" type using feature id (temporary dictionary)
        bldg_and_ghe_by_id = dict()
        ...
        thermal_connectors_by_startFeatureId = dict()
        # returns "ThermalConnector" endFeatureId using id (temporary dictionary)
        thermal_connectors_by_id = dict()
        ...
        for thermal_connector_2 in self.gj.thermal_connectors:
            ...
            thermal_connector_startFeatureId = thermal_connector_2.feature["properties"]["startFeatureId"]
            thermal_connector_endFeatureId = thermal_connector_2.feature["properties"]["endFeatureId"]
            # filling the data in the nested dictionary
            thermal_connectors_by_startFeatureId[thermal_connector_startFeatureId] = {"id": thermal_connector_id}
            # filling the data in the nested dictionary
            thermal_connectors_by_id[thermal_connector_id] = {"endFeatureId": thermal_connector_endFeatureId}

        # fills "Building" and "District System" data in connected_bldgs_and_ghes_by_order_in_loop dictionary (highest-level key of dictionary is order in loop)
        for num_in_loop in range(len(lst_bldgs_and_ghes)):
            if num_in_loop == 0:
                ...
                next_thermal_connector_in_loop = thermal_connectors_by_startFeatureId[next_bldg_or_ghe_in_loop]["id"]
                # filling the data in the nested dictionary
                connected_bldgs_and_ghes_by_order_in_loop[num_in_loop]["id"] = next_bldg_or_ghe_in_loop
                connected_bldgs_and_ghes_by_order_in_loop[num_in_loop]["type"] = bldg_and_ghe_by_id[next_bldg_or_ghe_in_loop]
                connected_bldgs_and_ghes_by_order_in_loop[num_in_loop]["id_ThermalConnector_out"] = next_thermal_connector_in_loop
        ...
        bldg_groups_by_num = dict_bldg_ghe_group_by_num(connected_bldgs_and_ghes_by_order_in_loop, num_of_bldg_groups, "num_bldgs_in_group", "Building", "building_group", "lst_bldg_ids_in_group", boolean_bldg_group_is_wrapping, "ghe_group_in")
        ghe_groups_by_num = dict_bldg_ghe_group_by_num(connected_bldgs_and_ghes_by_order_in_loop, num_of_ghe_groups, "num_ghes_in_group", "District System", "ghe_group", "lst_ghe_ids_in_group", boolean_ghe_group_is_wrapping, "building_group_in")
    ...
    def test_simulate_district_system(self):
        self.run_and_assert_in_docker(f'{self.district._scaffold.project_name}.Districts.DistrictEnergySystem',
