...
model buildingHPTrio
  ...
  extends Buildings.DHC.Loads.Combined.BaseClasses.PartialBuildingWithETS(
    COPHeaWat_nominal=3,
    redeclare DES_5G_Variable_Dymola.Loads.TimeSeriesBuilding bui(
      filNam=filNam,
      have_hotWat=true,
      ...
      delTAirCoo=10,
      delTAirHea=20),
    redeclare DES_5G_Variable_Dymola.Loads.ETS.HeatPumpTrio ets(
      proHeaWat(heaPum(use_eta_Carnot_nominal=use_eta_Carnot_nominal,
            etaCarnot_nominal=etaCarnot_heaPumHeaWat_nominal)),
      proHotWat(heaPum(use_eta_Carnot_nominal=use_eta_Carnot_nominal,
            etaCarnot_nominal=etaCarnot_heaPumHotWat_nominal)),
      proChiWat(chi(use_eta_Carnot_nominal=use_eta_Carnot_nominal,
            etaCarnot_nominal=etaCarnot_chiCHiWat_nominal)),
  ...
  parameter Real etaCarnot_heaPumHeaWat_nominal(unit="1")=0.494805
  ...
  parameter Real etaCarnot_heaPumHotWat_nominal(unit="1")=0.3
    "Carnot effectiveness (=COP/COP_Carnot) used during simulation if use_eta_Carnot_nominal = true"
    annotation (Dialog(group="Efficiency", enable=use_eta_Carnot_nominal));

  parameter Real etaCarnot_chiCHiWat_nominal(unit="1")=0.315046
    "Carnot effectiveness (=COP/COP_Carnot) used during simulation if use_eta_Carnot_nominal = true"
    annotation (Dialog(group="Efficiency", enable=use_eta_Carnot_nominal));
  ...
  parameter String filNam=
      "modelica://Mainz_Kastel_DES/Loads/Resources/Data/B1/modelica.mos"
  ...
  final parameter Modelica.Units.SI.HeatFlowRate QHot_flow_nominal(
    ...
    annotation (Dialog(group="Design parameter"));
  parameter Real COPChiWat_nominal(unit="1")=5
...
equation
  ...
  connect(bui.QReqHea_flow,loaHeaNor.u)
    annotation (Line(points={{20,6},{20,-6},{-218,-6},{-218,-100},{-202,-100}},color={0,0,127}));
  connect(bui.QReqCoo_flow,loaCooNor.u)
    annotation (Line(points={{26,6},{26,-4},{-220,-4},{-220,-140},{-202,-140}},color={0,0,127}));
