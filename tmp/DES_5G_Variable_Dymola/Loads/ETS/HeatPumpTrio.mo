within DES_5G_Variable_Dymola.Loads.ETS;
model HeatPumpTrio
  ...
  final parameter Modelica.Units.SI.MassFlowRate mConHotWat_flow_nominal(min=0)=
    abs(QHotWat_flow_nominal/cpBui_default/dT_nominal)
    "Condenser hot water mass flow rate"
    annotation (Dialog(group="Nominal condition"));
  ...
  final parameter Modelica.Units.SI.MassFlowRate mSerWat_flow_nominal(min=0) =
    max(proHeaWat.mCon_flow_nominal + mEvaHotWat_flow_nominal, proChiWat.mCon_flow_nominal)
  ...
  HeatPumpCooling                 proChiWat(
    chi(m2_flow_nominal=mChiWat_flow_nominal),
    ...
    final allowFlowReversal2=allowFlowReversalBui) "Chilled water heat pump"
  ...
  Buildings.Fluid.Delays.DelayFirstOrder volChiWatRet(
    ...
    nPorts=3) "Mixing volume representing building CHW primary" annotation (
      Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=-90,
        origin={86,-288})));
  ...
  Buildings.DHC.ETS.BaseClasses.Junction bypHeaWatSup(redeclare final package
      Medium = MediumBui, final m_flow_nominal=proHeaWat.mCon_flow_nominal*{1,-1,-1})
    "Bypass heating water (supply)" annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={100,260})));
  Buildings.DHC.ETS.BaseClasses.Junction bypHeaWatRet(redeclare final package
      Medium = MediumBui, final m_flow_nominal=proHeaWat.mCon_flow_nominal*{1,-1,1})
  ...
  Buildings.DHC.ETS.Combined.Subsystems.HeatPump proHeaWat(
    heaPum(m1_flow_nominal=mHeaWat_flow_nominal),
  ...
  Buildings.DHC.ETS.Combined.Subsystems.HeatPump proHotWat(
    heaPum(m1_flow_nominal=mConHotWat_flow_nominal),
    redeclare final package Medium1 = MediumBui,
    redeclare final package Medium2 = MediumSer,
  ...
  Buildings.Fluid.Sources.Boundary_pT sinSHW(redeclare final package Medium =
        MediumBui, nPorts=1) if have_hotWat "Sink for service hot water"
  ...
  Buildings.Fluid.Sensors.MassFlowRate senMasFloHeaWatPri(redeclare final
      package Medium = MediumBui, final allowFlowReversal=allowFlowReversalBui)
  ...
  Buildings.Controls.OBC.CDL.Reals.Subtract dTCHW "Chilled water DeltaT"
    annotation (Placement(transformation(extent={{-120,-220},{-140,-200}})));
  Buildings.Controls.OBC.CDL.Reals.Multiply loaCHW
    if have_varFloEva or have_varFloCon "Cooling load "
    annotation (Placement(transformation(extent={{-140,-280},{-120,-260}})));
  Buildings.Controls.OBC.CDL.Reals.MultiplyByParameter capFloCHW(final k=
        cpBui_default) if have_varFloEva or have_varFloCon "Capacity flow rate"
  ...
  Buildings.DHC.ETS.BaseClasses.Junction bypChiWatSup(redeclare final package
      Medium = MediumBui, final m_flow_nominal=proChiWat.mEva_flow_nominal*{1,-1,
        -1}) "Bypass cooling water (supply)" annotation (Placement(
        transformation(
        extent={{-10,10},{10,-10}},
        rotation=0,
        origin={104,-324})));
  Buildings.DHC.ETS.BaseClasses.Junction bypChiWatRet(redeclare final package
      Medium = MediumBui, final m_flow_nominal=proChiWat.mEva_flow_nominal*{1,-1,
        1}) "Bypass cooling water (return)" annotation (Placement(
        transformation(
        extent={{10,-10},{-10,10}},
        rotation=90,
        origin={64,-250})));
  Buildings.Fluid.Sensors.MassFlowRate senMasFloChiWatPri(redeclare final
      package Medium = MediumBui, final allowFlowReversal=allowFlowReversalBui)
...
equation
  ...
  connect(PPumCooTot.y, PPumTot.u[2]) annotation (Line(points={{212,380},{216,380},
          {216,400.5},{218,400.5}},
                                color={0,0,127}));
  connect(volMix_a.ports[1], swiFlo.port_bSup) annotation (Line(points={{-261.5,
          -360},{-6,-360},{-6,-370}},
                                color={0,127,255}));
  ...
  connect(bypHeaWatRet.port_2, volHeaWatRet.ports[1]) annotation (Line(points={{90,234},
          {80,234},{80,178.667}},             color={0,127,255}));
  ...
  connect(proHeaWat.port_b2, volMix_b.ports[3]) annotation (Line(points={{-10,208},
          {-16,208},{-16,194},{260.5,194},{260.5,-360}},
                                                 color={0,127,255}));
  connect(volMix_a.ports[3], proHeaWat.port_a2) annotation (Line(points={{-259.5,
          -360},{-259.5,200},{14,200},{14,208},{10,208}},
                                                        color={0,127,255}));
  ...
  connect(proHeaWat.PPum, PPumHeaTot.u[1]) annotation (Line(points={{12,214},{172,
          214},{172,419.5},{188,419.5}},
                                     color={0,0,127}));
  connect(volMix_a.ports[4], proHotWat.port_a2) annotation (Line(points={{-258.5,
          -360},{-258.5,20},{14,20},{14,28},{10,28}},           color={0,127,
          255}));
  connect(proHotWat.port_b2, volMix_b.ports[4])
    annotation (Line(points={{-10,28},{-16,28},{-16,12},{261.5,12},{261.5,-360}},
                                                           color={0,127,255}));
  ...
  connect(sinSHW.ports[1], proHotWat.port_b1) annotation (Line(points={{-50,60},
          {16,60},{16,40},{10,40}},    color={0,127,255}));
  connect(gai.y, div1.u2) annotation (Line(points={{-98,0},{-80,0},{-80,-20},{-120,
          -20},{-120,-46},{-102,-46}},                color={0,0,127}));
  connect(QReqHotWat_flow, div1.u1) annotation (Line(points={{-320,-120},{-290,-120},{-290,
  ...
  connect(senTHeaWatRet.port_b, bypHeaWatRet.port_1) annotation (Line(points={{30,300},
          {120,300},{120,234},{110,234}},      color={0,127,255}));
  ...
  connect(bouChiWat.ports[1], volChiWatRet.ports[1]) annotation (Line(points={{50,-288},
          {64,-288},{64,-286.667},{76,-286.667}},       color={0,127,255}));
  connect(zer1.y, masFloHea.u2) annotation (Line(points={{-19,-234},{-6,-234},{-6,
          -252}},              color={0,0,127}));
  connect(proHotWat.mEva_flow, masFloHea.u2) annotation (Line(points={{12,31},{16,
          31},{16,-234},{-6,-234},{-6,-252}}, color={0,0,127}));
  ...
  connect(senMasFloHeaWat.m_flow, priOve.u1) annotation (Line(points={{-240,271},
          {-240,302},{-72,302},{-72,286},{-62,286}}, color={0,0,127}));
  ...
  connect(uCoo, proChiWat.uEna) annotation (Line(points={{-320,120},{-264,120},{
          -264,-344},{-138,-344},{-138,-345},{-12,-345}},  color={255,0,255}));
  connect(volChiWatRet.ports[2], proChiWat.port_a2) annotation (Line(points={{76,
          -288},{76,-330},{10,-330}}, color={0,127,255}));
  connect(proChiWat.PPum, PPumCooTot.u[1]) annotation (Line(points={{12,-336},{22,
          -336},{22,22},{188,22},{188,380}},    color={0,0,127}));
  connect(senTChiWatSup.T, dTCHW.u2) annotation (Line(points={{140,-269},{140,-216},
          {-118,-216}},       color={0,0,127}));
  connect(senTChiWatRet.T, dTCHW.u1) annotation (Line(points={{40,-131},{40,-204},
          {-118,-204}},       color={0,0,127}));
  connect(TChiWatSupSet, proChiWat.TSupSet) annotation (Line(points={{-320,0},{-268,
          0},{-268,-340},{-32,-340},{-32,-339},{-12,-339}}, color={0,0,127}));
  connect(loaCHW.u2,capFloCHW. y)
    annotation (Line(points={{-142,-276},{-158,-276}}, color={0,0,127}));
  connect(dTCHW.y, loaCHW.u1) annotation (Line(points={{-142,-210},{-152,-210},{
          -152,-264},{-142,-264}},             color={0,0,127}));
  connect(senMasFloChiWat.m_flow, capFloCHW.u) annotation (Line(points={{-240,-131},
          {-240,-276},{-182,-276}}, color={0,0,127}));
  connect(proChiWat.mCon_flow, swiFlo.mRev_flow) annotation (Line(points={{12,-333},
          {-22,-333},{-22,-384},{-12,-384}}, color={0,0,127}));
  connect(loaCHW.y, conFloConCHW.loa)
    annotation (Line(points={{-118,-270},{-102,-270}}, color={0,0,127}));
  connect(conFloConCHW.m_flow, priOve1.u1) annotation (Line(points={{-78,-270},{
          -68,-270},{-68,-264},{-62,-264}}, color={0,0,127}));
  connect(senMasFloChiWat.m_flow, priOve1.u2) annotation (Line(points={{-240,-131},
          {-240,-300},{-68,-300},{-68,-276},{-62,-276}}, color={0,0,127}));
  connect(priOve1.y, proChiWat.m2_flow) annotation (Line(points={{-38,-270},{-32,
          -270},{-32,-336},{-12,-336}}, color={0,0,127}));
  connect(souColWat.ports[1], proHotWat.port_a1) annotation (Line(points={{-34,-42},
          {-28,-42},{-28,40},{-10,40}}, color={0,127,255}));
  connect(TColWat, souColWat.T_in) annotation (Line(points={{-320,-80},{-68,-80},
          {-68,-38},{-56,-38}}, color={0,0,127}));
  connect(div1.y, proHotWat.m1_flow) annotation (Line(points={{-78,-40},{-70,-40},
          {-70,34},{-12,34}}, color={0,0,127}));
  connect(volMix_b.ports[2], proChiWat.port_a1) annotation (Line(points={{259.5,
          -360},{-258,-360},{-258,-342},{-10,-342}}, color={0,127,255}));
  connect(proChiWat.port_b1, volMix_a.ports[2]) annotation (Line(points={{10,-342},
          {-260.5,-342},{-260.5,-360}},
                                      color={0,127,255}));
  connect(proChiWat.PHea, PCoo) annotation (Line(points={{12,-339},{112,-339},{
          112,-336},{252,-336},{252,40},{320,40}}, color={0,0,127}));
  connect(proChiWat.mCon_flow, mCoo_flow) annotation (Line(points={{12,-333},{
          128,-333},{128,-330},{242,-330},{242,-180},{320,-180}}, color={0,0,
          127}));
  connect(senTChiWatRet.port_b, bypChiWatRet.port_1) annotation (Line(points={{50,
          -120},{64,-120},{64,-240}}, color={0,127,255}));
  connect(bypChiWatRet.port_2, volChiWatRet.ports[3]) annotation (Line(points={{64,-260},
          {64,-286},{76,-286},{76,-289.333}},          color={0,127,255}));
  connect(senMasFloChiWatPri.port_a, proChiWat.port_b2) annotation (Line(points
        ={{40,-324},{-14,-324},{-14,-330},{-10,-330}}, color={0,127,255}));
  connect(senTChiWatSup.port_a, bypChiWatSup.port_2) annotation (Line(points={{130,
          -280},{120,-280},{120,-324},{114,-324}}, color={0,127,255}));
  connect(bypChiWatSup.port_1, senMasFloChiWatPri.port_b)
    annotation (Line(points={{94,-324},{60,-324}}, color={0,127,255}));
  connect(bypChiWatRet.port_3, bypChiWatSup.port_3) annotation (Line(points={{74,
          -250},{104,-250},{104,-314}}, color={0,127,255}));
  annotation (
...
<a href=\"https://github.com/lbl-srg/modelica-buildings/issues/1097\">issue 1097</a>.
    ...
    Icon(coordinateSystem(preserveAspectRatio=false, extent={{-300,-300},{300,300}}),
        ...
        Polygon(
          ...
          fillColor={255,255,255},
          fillPattern=FillPattern.Solid),
        Rectangle(extent={{-260,140},{260,-142}}, lineColor={95,95,95})}));
end HeatPumpTrio;
